...
class FormEvent
{
    ...
    public function get($id=0)
    {
        ...
        if((int)$id && ($row = $this->model->get($this->entity)->getRowById($id))) {
            ...
            $venues = $this->model->get('Venue')->getAll(['linked_to'=>$this->entity, 'linked_to_id'=>$row->getId()]);
        ...
        $form_config['fields']['medias'] = [
            ...
            'options'=>[
                'entry_type' => UIFormType::class,
                'entry_options' => [
                    'label' => $this->translator->trans('fld_event_medias_media', [], 'admin'), 
        ...
        $form_config['fields']['venues'] = [
            ...
            'options'=>[
                'entry_type' => UIFormType::class,
                'entry_options' => [
                    'label' => $this->translator->trans('fld_venues_venue', [], 'admin'), 
                    'attr'=>['class'=> 'collection_entry', 'required'=>true] , 
                    'translator'=>$this->translator,
                    'ui_config' => $this->ui_config,
                    'form_config' => $this->ui_config['entity']['App\Entity\Venue']['form'],
                    'data_class' => 'App\Entity\Venue',
                ],
                'allow_add' => true,
                'allow_delete' => true,
                'by_reference' => false,
                'prototype' => false,
                'data'=> $venues,
                'label' => $this->translator->trans('fld_venues', [], 'admin'),
                'mapped'=>false,
                'attr'=>['prototype'=>$this->form->getPrototype('venues', 'App\Entity\Venue'), 'class'=>'collection_type'],
            ],
        ];
    ...
    public function handle($form)
    {
        ...
        if ($form->isSubmitted() && $form->isValid()) {
            ...
            try {
                $message = $row->getId() ? 'event_updated' : 'event_inserted';
                ...
                # Venues
                if($venues = $form->get('venues')->getData()) {
                    foreach($venues as $i=>$venue) {
                        $this->model->get('Venue')->mode('admin')->persist($venue);
                    }
                }
                ...
                $this->model->get($this->entity)->mode('admin')->unlink([$row->getId()], 'App\Entity\Venue');
                ...
                # Link Venues
                foreach($venues as $i=>$venue) {
                    $this->model->get($this->entity)->mode('admin')->link([$row->getId()], 'App\Entity\Venue', [$venue->getId()]);
                }
