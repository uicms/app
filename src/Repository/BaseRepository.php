...
class BaseRepository extends ServiceEntityRepository
{
    ...
    $this->default_locale = $parameters->get('locale');
		foreach($this->ui_config['entity'] as $entity_name=>$entity) {
    ...
    protected function setRowData($row, $params=array()) 
    {
        ...
        if(null !== $row) {
            # Meta
            $row->_name = $this->ui_config['row_default_name'];
            $row->_file = null;
            $row->_thumbnail = null;
            $row->_text = null;
            $row->_links = array();
            ...
            if(is_object($row->_name) && isset($this->ui_config['entity'][$this->name]['form']['fields'][$this->config['name_field']]['options']['class'])) {
                $foreign_entity_name = $this->ui_config['entity'][$this->name]['form']['fields'][$this->config['name_field']]['options']['class'];
    ...
    {
        foreach($this->ui_config['admin']['pages'] as $page) {
	...
	public function getFields($params=array())
	{
		...
		foreach($result as $i=>$field_name) {
			...
			foreach($this->ui_config['entity'][$this->name]['form']['fields'] as $form_field_name=>$form_field) {
        ...
        if($this->isTranslatable()) {
    		...
    		foreach($result as $i=>$field_name) {
    			...
    			$data = array('name'=>$field_name);
    			foreach($this->ui_config['entity'][$this->name]['form']['translations'] as $form_field_name=>$form_field) {
    				if($field_name == $form_field_name) {
    					$data['form'] = $form_field;
    				}
    			}
	...
	public function persist($data, $current=null)
	{
		if($this->security->isGranted('ROLE_PERSIST')) {
            ...
            if($current) {
    			foreach($this->ui_config['entity'][$this->name]['form']['fields'] as $j=>$form_field) {		
	...
	public function position($selection, $position, $params=array())
	{
		if($this->security->isGranted('ROLE_PERSIST')) {
            ...
            $paginator = new \App\Service\Paginator($params['offset'], $params['limit'], $total);
